...
public class SlidingLayer extends FrameLayout {
    ...
    public void setTopNeihorView(View parentView ,View topNeigborViewview) {
        mTopNeigborView=topNeigborViewview;
        mParentView = parentView;
    }


    private int computeOffsetDistance() {
        return mParentView.getBottom() - mTopNeigborView.getBottom();
    }
    ...
    private int determineNextStateForDrag(final int scrollX, final int scrollY, final int velocityX,
        ...
        if (isOverThreshold) {
            ...
            int openedThreshold = (panelSize) / 2;
    ...
    private int[] getDestScrollPosForState(int state) {
        ...
        if (state == STATE_OPENED) {
            ...
            int layerOffset = state == STATE_CLOSED ? computeOffsetDistance() :0;
            Log.i("xxzz", "-----------:" + layerOffset);
            switch (mScreenSide) {
                case STICK_TO_RIGHT:
