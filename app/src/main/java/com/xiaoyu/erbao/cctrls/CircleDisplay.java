...
public class CircleDisplay extends View implements OnGestureListener {
    ...
    private int mInnerRadiusOffset;

    private Paint mOuterCirclePaint;
    private Paint mValueArcPaint;
    private Paint mInnerCirclePaint;
    private Paint mTextPaint;
    private Paint mTextPaintLittle;
    ...
    private void init() {
        ...
        mInnerRadiusOffset = getResources().getDimensionPixelSize(R.dimen.circle_progress_radius_offset);

        mOuterCirclePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mOuterCirclePaint.setStyle(Style.FILL);
        mOuterCirclePaint.setColor(getResources().getColor(R.color.fp_circle_outer_color));

        mValueArcPaint = new Paint();
        mValueArcPaint.setAntiAlias(true);
        mValueArcPaint.setStyle(Style.STROKE);
        mValueArcPaint.setDither(true);
        mValueArcPaint.setStrokeWidth(mInnerRadiusOffset);
        mValueArcPaint.setStrokeJoin(Paint.Join.ROUND);
        mValueArcPaint.setStrokeCap(Paint.Cap.ROUND);
        mValueArcPaint.setColor(getResources().getColor(R.color.fp_circle_progress_color));
        ...
        mTextPaint.setColor(getResources().getColor(R.color.fp_circle_text_color));
        mTextPaint.setTextSize(getResources().getDimension(R.dimen.circle_progress_text_size));

        mTextPaintLittle = new Paint(Paint.ANTI_ALIAS_FLAG);
        mTextPaintLittle.setStyle(Style.STROKE);
        mTextPaintLittle.setTextAlign(Align.CENTER);
        mTextPaintLittle.setColor(getResources().getColor(R.color.fp_circle_text_color));
        mTextPaintLittle.setTextSize(getResources().getDimension(R.dimen.circle_progress_littel_text_size));

        mDrawAnimator = ObjectAnimator.ofFloat(this, "phase", mPhase, 1.0f);
    ...
    private void drawText(Canvas c) {
        String txt1 = String.valueOf((int) (mValue * mPhase));
        Rect txt1Bounds = new Rect();
        mTextPaint.getTextBounds(txt1, 0, txt1.length(), txt1Bounds);
        String txt2 = mUnit;
        Rect txt2Bounds = new Rect();
        mTextPaintLittle.getTextBounds(txt2, 0, txt2.length(), txt2Bounds);
        String txt3 = getResources().getString(R.string.common_string_hyl);
        Rect txt3Bounds = new Rect();
        mTextPaintLittle.getTextBounds(txt3, 0, txt3.length(), txt3Bounds);
        c.drawText(String.valueOf((int) (mValue * mPhase)), getDiameter() / 2 - txt1Bounds.width() / 6, getDiameter() / 2 + txt1Bounds.height() / 3, mTextPaint);
        c.drawText(txt2, getDiameter()/2 +getResources().getDimension(R.dimen.circle_progress_littel_text_margin), getDiameter() / 2 + txt1Bounds.height() / 6, mTextPaintLittle);
        c.drawText(txt3, getDiameter() / 2, getDiameter() / 2 + txt1Bounds.height(), mTextPaintLittle);
    ...
    private void drawOuterCircle(Canvas c) {
        mOuterCirclePaint.setAlpha(mDimAlpha);
        float r = getRadius();
        c.drawCircle(getWidth() / 2, getHeight() / 2, r, mOuterCirclePaint);
    }

    // 内圆
    private void drawInnerCircle(Canvas c) {
        c.drawCircle(getWidth() / 2, getHeight() / 2, getRadius() - mInnerRadiusOffset, mInnerCirclePaint);
    }

    private void drawValueArc(Canvas c) {
        mValueArcPaint.setAlpha(255);
        mValueArcPaint.setStrokeCap(Paint.Cap.ROUND);
        float angle = mAngle * mPhase;
        c.drawArc(mCircleBox, mStartAngle, angle, false, mValueArcPaint);
    }


    private void setupBox() {
        float diameter = getDiameter();
        mCircleBox = new RectF(mInnerRadiusOffset / 2, mInnerRadiusOffset / 2, diameter - mInnerRadiusOffset / 2, diameter - mInnerRadiusOffset / 2);
    }
    ...
    public void setAnimDuration(int duration_millis) {
        mDrawAnimator.setDuration(duration_millis);
    }
    ...
    public void setValueArcColor(int color) {
        mValueArcPaint.setColor(color);
    }
